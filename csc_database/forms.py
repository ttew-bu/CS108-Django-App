...
class CreateVolunteerForm(forms.ModelForm):
    ...
    first_name = forms.CharField(label="First Name", required=True)#standard character field
    last_name = forms.CharField(label="Last Name", required=True)
    phone = formfields.PhoneNumberField(label="Phone Number", required=True)#custom field that only works with real and validated phone numbers
    email = forms.EmailField(label="Email Address", required=True)#emailfield specificies what can go in here
    bu_id = forms.CharField(label="BU ID Number", required=True, max_length=9)#charfield allows for both numbers and letters, 9 is the length of a UID starting with U
    class_year = forms.ChoiceField(label="Class Year", choices=[(x,x) for x in range(2018,2025)], required=True)#use the choices in a lc to use relevant class years for volunteers
    pref_service = forms.CharField(label="Preferred Service Area", widget=forms.Select(choices=service_categories), required=True)#use the service categories from above so that the list lines up between models
...
class CreatePartnerForm(forms.ModelForm):
    '''A form to add volunteers to the database'''
    cp_name = forms.CharField(label="Partner Name", required=True, help_text='Please put in the name of the CP')#basic Charfield
    cp_address= forms.CharField(label="Partner Address", required=True, help_text='Please put in the street address for the CP')#basic Charfield 
    cp_image= forms.ImageField(label="Logo", required=False, help_text='Please Attach a JPG, PNG, or other acceptable Image Format Here')#basic image upload
    cp_mission = forms.CharField(label="Mission", required=True, help_text='Please write a brief mission statement for the CP')#basic charfield since this will just display on the pages
    cp_type = forms.ChoiceField(label="Service Area", choices=service_categories, required=True, help_text='Please select the applicable service area from the list')#service categories line up with above form so that the models can interact
...
class CreateEventForm(forms.ModelForm):
    ...
    event_name = forms.CharField(label ="Name", required=True)#basic charfield for the name
